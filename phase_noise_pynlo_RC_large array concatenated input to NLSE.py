...
#%%
# #---- Run Sim
# sim = pynlo.model.NLSE(pulse, mode2) # INSTEAD OF SM_UPE
# #---- Estimate step size
# local_error = 1e-6
...
# start = time.time()
# new_pulse, z, a_t, a_v = sim.simulate(length, dz=dz, local_error=local_error, n_records=100, plot="frq")#25e-3 for 1micron pump
# end = time.time()
# print('Time elapsed is ',end - start)
# plt.figure('SC spectrum')
# plt.semilogy(new_pulse.v_grid*1e-12, new_pulse.p_v)
#%%
...
for i in range(tot_no):
    ...
    phase_noise = rng.normal(0,np.sqrt(var_phase1_new)*1)/(2*np.pi * fr) 
...
#%%
inp_p_a_t.shape = inp_p_a_t.size # concatenating

# adding common phase noise contribution from CW laser
phase_noise_CW = np.cumsum(np.sqrt(var_pn_CW) * np.random.randn(n_points*tot_no))
inp_p_a_t*= np.exp(1j*phase_noise_CW)

inp_tr_a_v = fft.fftshift(fft.fft(fft.ifftshift(inp_p_a_t),fsc = pulse.dt)) 
...
#%%
'''
concatenating pulse train
'''
e_p = e_p * tot_no

n_points = tot_no*n_points
pt = pynlo.light.Pulse.Sech(n_points, v_min, v_max, v0, e_p, t_fwhm)
print("Frq Res: {:.3g} GHz".format(pt.dv * 1e-3))
pt.a_t = inp_p_a_t
...
g3_v = pynlo.utility.chi3.gamma_to_g3(pt.v_grid, gamma_spline(pt.v_grid))
...
beta_v = pynlo.utility.chi1.n_to_beta(pt.v_grid, n_eff_spline(pt.v_grid))

#---- Mode
mode2 = pynlo.medium.Mode(pt.v_grid, beta_v, g3=g3_v) # media
#%%
#---- Run Sim for input pulse train
sim = pynlo.model.NLSE(pt, mode2) # INSTEAD OF SM_UPE
#---- Estimate step size
local_error = 1e-6
dz = sim.estimate_step_size(local_error=local_error)
length = 5e-3
...
start = time.time()
new_pulse1, z1, a_t1, a_v1 = sim.simulate(length, dz=dz, local_error=local_error, n_records=100, plot=None)#25e-3 for 1micron pump
end = time.time()
print('Time elapsed is ',end - start) 

#%%
v_new = np.linspace(v_min, v_max, n_points)
idx1 = np.argmin(np.abs(v_new*1e-12 - c*1e-3/1500)) # c*1e-3/1570
idx2 = np.argmin(np.abs(v_new*1e-12 - c*1e-3/1510))

sc = a_v1
# np.save(dir + "sc_timjitter_var_phase1_-v0_CWphasenoise" + str(CW_LW) + "_2rt" + str(np.log2(tot_no*n_points)) + "_SiN2500nm.npy", sc)

plt.plot(v_new[idx2:idx1]* 1e-12, np.abs(sc[-1][idx2:idx1])**2, 'o-')
# plt.semilogy(v_new* 1e-12, np.abs(inp_tr_a_v)**2, 'o-')
# plt.semilogy(v_new* 1e-12, np.abs(sc[-1])**2, 'o-')
